...
- UI 동작이 여러 방향으로 해석된다.
- 재현 조건이 부족하다.
- assignee, milestone, Project 같은 필수 메타데이터를 신뢰할 수 없다.
...
`issue-management`에는 중복 검색, canonical 통합, 하위 이슈 분리, `blocked-by`, `Issue Review`, 본문 템플릿, Project 상태 검증 규칙이 이미 들어 있다.
