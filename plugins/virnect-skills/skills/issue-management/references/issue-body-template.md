...
| 항목 | 내용 |
|------|------|
| 증상 repo | <증상이 관측된 product/module repo> |
| root-cause repo | <확정 repo 또는 후보 repo 목록> |
| 근거 파일/class/package | <검색한 파일, class, symbol, package명> |
| 관련 active PR/issue | <기존 PR/issue 후보와 판단> |
| product 통합 검증 책임 | <product repo에서 검증해야 할 범위> |
...
- 신규 원인 소유권 검토 `Issue Review` 이슈에는 `사용자 확인 필요 사항` 섹션을 넣을 수 있다. 이때 질문은 root-cause ownership 해소에만 한정하고, 구현 계획과 완료 기준을 확정된 것처럼 쓰지 않는다.
- 중복 또는 동일 문제 이슈가 없으면 `관련 이슈 통합` 섹션을 제거한다.
- confirmed product-only 이슈이면 `원인 및 저장소 소유권` 섹션을 제거할 수 있다.
